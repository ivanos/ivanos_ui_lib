...
/***/ function(module, exports, __webpack_require__) {
	...
	var logView = {
	    reset: function reset() {
	        var $logsContainer = (0, _jquery2['default'])(".log-list-container>div.list-group");
	        ...
	        $logsContainer.on("click", "a", null, function () {
	            ...
	            logView.displayLog((0, _jquery2['default'])(this).data("log"));
	        });
	
	        var $logContainer = (0, _jquery2['default'])(".json-viewer-container>div");
	        $logContainer.off();
	        $logContainer.children().remove();
	
	        $logContainer.on("click", ".entry .key", null, function () {
	            (0, _jquery2['default'])(this).parent(".entry").toggleClass("collapsed");
	        });
	    },
	
	    displayLogTimestamp: function displayLogTimestamp(log) {
	        return "<p class='list-group-item-text'>" + log.date + " " + log.time + "</p>";
	    },
	
	    displayLogSeverity: function displayLogSeverity(log) {
	        return '<div class="badge alert-' + getSeverityPostfix(log.severity) + '">' + log.severity + '</div>';
	    },
	
	    displayLogMessage: function displayLogMessage(log) {
	        return "<h4 class='list-group-item-heading'>" + log.message + "</h4>";
	    },
	
	    displayLogEntry: function displayLogEntry(log) {
	        return this.displayLogSeverity(log) + this.displayLogTimestamp(log) + this.displayLogMessage(log);
	    },
	
	    addLogEntry: function addLogEntry(log) {
	        var $entry = (0, _jquery2['default'])("<a href='#'>").addClass("list-group-item").addClass("list-group-item--" + getSeverityPostfix(log.severity)).html(this.displayLogEntry(log)).data({ log: log });
	        (0, _jquery2['default'])(".log-list-container>div").prepend($entry);
	    },
	
	    displayLog: function displayLog(log) {
	
	        var $logContainer = (0, _jquery2['default'])(".json-viewer-container > div"),
	            logContent = ['<div class="panel panel-default panel-', getSeverityPostfix(log.severity), '">', '<div class="panel-heading">', '<h3 class="panel-title">', '<span class="severity-label label label-', getSeverityPostfix(log.severity), '">', log.severity, '</span> ', '<span>', log.message, '</span></h3>', '</div>', '<div class="panel-heading">', '<div class="panel-title">', this.displayLogTimestamp(log), '</div>', '</div>', '<div class="panel-body">', '<pre>', getHTMLConverter(log.metadata)(log.metadata), '</pre>', '</div>', '</div>'].join("");
	...
	function objToHTML(obj) {
	    ...
	    return keys.reduce(function (res, key, index, items) {
	        ...
	        return res += "<span class='entry'><span class='key'>\"" + key + "\"</span><span class='col'>:</span><span class='val'>" + converter(token) + "</span>" + (index + 1 == items.length ? "" : comma) + "</span>";
